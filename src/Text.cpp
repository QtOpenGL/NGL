...
Text::Text( const QFont &_f)  noexcept
{
  ...
  std::unordered_map <int,std::shared_ptr<AbstractVAO>> widthVAO;
  ...
  for(char c=startChar; c<=endChar; ++c)
  {
   ...
   // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    ...
    if (widthVAO.find(width) ==widthVAO.end())
    {
        ...
        std::shared_ptr<AbstractVAO> vao=VAOFactory::createVAO("simpleVAO",GL_TRIANGLES);
...
//---------------------------------------------------------------------------
void Text::renderText( float _x, float _y,  const QString &text )  noexcept
{
